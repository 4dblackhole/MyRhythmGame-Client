...
이동합니다. 16분음표 중심 간격은 기본 노트 이미지 지름의 85%이며, 차트상
1/16 온음표 간격마다 같은 화면 거리를 사용합니다. 따라서 별도 스크롤 배율 효과가
없으면 BPM이 낮아질수록 노트 이동 속도도 비례해서 낮아지고, BPM 변경 전후의
노트 간격은 일정합니다. 노트별 `NoteSpeed`·`ScrollSpeed` 배율은 이 기본 거리에
곱합니다. 판정과 오디오 예약은 기존의 정수 마이크로초 시각을 계속 사용합니다.
차트의 양수 delay 동안에는 화면의 박자 위치가 멈춥니다. 창 크기가 바뀌어도
노트와 롱노트 tick 간격은 유지되며, 출현 위치와 표시 범위만 새 Lane 길이에 맞춥니다.
시작 대기 시간은 최소 2초이고, 낮은 BPM이나 넓은 화면에서 첫 노트가 화면 밖에서
들어올 시간이 더 필요하면 그만큼 늘어납니다.
